...
menu.add_text("info", "THIS IS ONLY A SEMI-FIX")
menu.add_text("info", "enemies and local models cannot be overrided")
menu.add_text("info", "if you have any idea on how to fix api bug contact me")
menu.add_text("info", "My Discord: NoxLoveYa#5666#5666")
...
local attachments_keep_original = menu.add_checkbox("local player", "Keep original model")
...
callbacks.add(e_callbacks.DRAW_MODEL, function(ctx)
   ...
   if (model:find("_dropped.mdl") and model:find("weapons/w") and not model:find("arms") and not model:find("ied_dropped")) then--or (ctx.entity and is_attachment_entity(ctx.entity, entity_list.get_local_player())) then
         ...
         if attachments_base_chams_master:get() or attachments_overlay_chams_master:get(1) or attachments_overlay_chams_master:get(2) then
            if not attachments_keep_original:get() then
               ctx.override_original = true
            end
         end
