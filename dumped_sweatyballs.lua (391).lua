...
local references = {
    lua = {
        active_tab = menu.add_list( "Sweatyballs", "[AA] Condition", conditions_names ),
        inAir = menu.add_checkbox( "Sweatyballs", "[ANIM] Freeze legs in air" ),
        onGround = menu.add_checkbox( "Sweatyballs", "[ANIM] Break legs on ground" ),
        onLand = menu.add_checkbox( "Sweatyballs", "[ANIM] Pitch zero on land" ),
        lean = menu.add_checkbox( "Sweatyballs", "[ANIM] Lean model" ),
    },
    menu = {
        antiaim_list = {
...
local function indicators()
    ...
    render.text(font_inds, "sweatyballs", vec2_t(x + 2, y + 43), indi_color, true)
...
local enableSwag = menu.add_checkbox(" | general", "enable logger", false)
local printtoConsole = menu.add_checkbox(" | general", "print logs to console", false)

local amountonscreen = menu.add_slider(" | modifiers", "amount of logs on screen at one time", 1, 10)
local timelogs = menu.add_slider(" | modifiers", "time before logs fade", 0, 10)
local fontstyle = menu.add_selection(" | modifiers", "font style", {"normal", "bold"})
...
function onHit(e)
    local hitString = string.format("[sweatyballs]Hit %s in the %s [%s] for %s [%s] (hc: %s bt: %s)", e.player:get_name(), hitgroup_names[e.hitgroup + 1] or '?', hitgroup_names[e.aim_hitgroup + 1], e.damage, e.aim_damage, math.floor(e.aim_hitchance).."%", e.backtrack_ticks)
...
function onMiss(e)
    local missString = string.format("[sweatyballs]Missed %s %s due to %s [%s] (hc: %s bt: %s)", e.player:get_name().."'s", hitgroup_names[e.aim_hitgroup + 1] or '?', e.reason_string, e.aim_damage, math.floor(e.aim_hitchance).."%", e.backtrack_ticks)
